...
%
% MAQUINADERIVAPRIMITIVA Máquina para calcular derivadas e primitivas analiticamente (solução extata) e numericamente (solução aproximada)
%
...
testeFunc=MException('MATLAB:pbAnaliticamente_Callback:badG','Introduza uma função em "x"!');
...
% Hint: delete(hObject) closes the figureSN=questdlg('Quer sairM?','SAIRM','Sim','Não','Sim');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
msgbox({'        Foi-nos proposto pelo professor Arménio Correia, o desenvolvimento duma aplicação em MATLAB denominada de “Máquina para Derivação e Integração”. Esta que irá determinar a derivada duma função num conjunto de pontos.';
'';
'        A atividade foi realizada por Carolina Proença, Henrique Barrada, Mariana Cação e Rafael Fonseca, alunos da Licenciatura em Engenharia Informática, no Instituto Superior de Engenharia de Coimbra (ISEC).';
''}, 'Sobre', 'modal');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
msgbox({'        Em muitas situações, o cálculo analítico da derivada revela-se moroso ou mesmo impraticável, surgindo a necessidade de considerar uma aproximação numérica. A solução é aplicar uma das fórmulas de diferenças finitas: progressivas, regressivas ou centradas.';''}, 'Método de Euler', 'modal');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
msgbox({'        Na matemática, especialmente na análise numérica, existe uma grande família de algoritmos, cujo principal objetivo é aproximar o valor de um dado integral definida duma função sem o uso duma expressão analítica para a sua primitiva.';''}, 'Método de Euler', 'modal');
